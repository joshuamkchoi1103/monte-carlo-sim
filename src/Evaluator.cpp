...
int Evaluator::hasStraight(uint64_t mask) {
    ...
    for (int i = 12; i >= 4; --i) {
        ...
        if ((ranks & straightMask) == straightMask) return i; 
    }
    
    if ((ranks & 0x100F) == 0x100F) return 3; 
...
uint32_t Evaluator::checkMultiples(uint64_t mask) {
    ...
    if (trip1 != -1) {
        if (trip2 != -1) return buildScore(6, trip1, trip2); 
...
uint32_t Evaluator::evaluate(const Bitboard& board) {
    ...
    if (flushSuitMask) {
        ...
        if (straightTop != -1) return buildScore(8, straightTop); 
    ...
    if (straightTop != -1) return buildScore(4, straightTop); 
